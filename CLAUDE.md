...
## 第二章（demo2.mp4 = 纪念碑谷2 开篇）——分析完成，实现为脚手架 ⏳

**已确认事实**（逐帧分析 t=0-111，1fps 全帧 + 关键段全分辨率）：
- 出自**纪念碑谷2**（片尾通知栏可见）：主角 Ro（橙裙深发）+ **红衣小孩**沿链
  跟随（落后约 1.5 单位）。白昼素净背景（≈rgb(251,249,247)），无星/月。
- 结构：方形井坑（黑）+ 石环 + 之字悬浮石径 + 四面等高墙体的塔楼 +
  墙顶环廊（等高相连）+ FL 端平台 + 中央柱 + 柱顶橙色转子。
- 机关：**行走桥绕竖直轴 90° 转动**（点击转子触发，~0.9s 缓动），
  桥对接 FL 拱门 ↔ FR 橙框门洞；转动时桥上角色被载运（分数保持，
  端点随角度变化）。t=59 起**后角塔段升起**并随总成旋转（未实现）。
- 结局：Ro 登上塔段顶 → 全结构沉入坑中 → 金色花绽放（未实现）。
- **关键发现：第二章相机仰角 ≈52.6°**（井坑投影高宽比 0.794 →
  sin(e)=0.794），**不是** fable 的 35.26°（0.577）——投影体系不同，
  第一章相机参数不可复用；K≈28.6px/单位（艾达 30px=1.05 单位）✓。
  墙高≈7.3-7.7 单位、坑边≈5.9、门高≈1.9。
- 已核路线（t=0-45）：之字径→井沿外沿→基座梯→拱门隧道（穿墙）→
  行走桥（0°）→FR 门洞→墙内换位→FR 外墙角梯（贴前角上行）→
  墙顶环廊（FR→BR→BL→FL）→FL 端平台。t=20-23 转子两连转为演示动作。

**实现现状（2026-09-25 二次迭代，可用）**：相机模型已解决——引擎支持
  chapter.cameraHalfH/cameraDir 覆盖，第二章实测 **e≈51-52°、K≈28.6、
  halfH=22.4、dir=(0.434,0.788,0.434)**；结构修正：**塔楼前角敞开**
  （FL/FR 墙在前角前截断于 4.2，前角无墙——这是桥面/坑内可见的前提），
  中央柱在对角线上 (3.33,3.33)（拱门 x≈0.9/门洞 z≈0.9 的连线），
  桥面高 8.2（坑口上方 2/3 墙高处），拱门=通高隧道（井沿→桥面，楼梯在内
  可见），红衣小孩沿链落后 2.1 跟随，镜头随爬升上移，章节标题/提示
  按 META 动态化。逐状态对比证据：shots/ch2_vs_video_start.png、
  ch2_vs_video_beam.png（左=实现，右=视频帧，用户视口 393×852）。
  chapters/chapter2.js 已建（day 天空/井坑/石径/
四面墙/双层旋转桥/转子/跟随小孩/相机随爬升上移/#ch=2 切换），但
**相机模型仍是 fable 的 35.26°**——与视频的 52.6° 不符，导致桥面被
高墙遮挡、比例观感不对。**下一步**：① engine.placeCamera 支持
chapter.cameraDir（elevation 52.6° 的 right/up/dir 三元组）；
② 在新相机模型下重新标定全部几何（桥面高度、门洞、基座梯级数）；
③ 补升塔段与沉塔结局；④ 逐状态与视频帧对比验收。
门禁：ch1 全绿不受影响（默认 #ch=1）。
