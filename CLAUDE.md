...
## 第二章（demo2.mp4 = 纪念碑谷2 开篇）——第一阶段可玩（fable 思路完全重做版）⏳

**2026-09-25 按用户要求参照 fable 思路完全重做**（此前两版失败根因：未按
fable 方法标定，尺寸/比例靠猜导致墙高比例错误、内容全遮挡）。

**标定（fable 式：单锚点闭联）**：锚点 = Ro 像素高 30px（同第一章人物
1.05 单位）。井坑投影高宽比 0.794 → 仰角 e=51°。ppu = 30/(1.05·cos51°)
= 45.4 → CAM_HALF_H = 14.1。实测换算：坑口 238px→3.7 单位、墙外立面带
220px→墙高 6.9、门洞 92px→3.2、桥面 3.2、转子中心 5.0。
视错觉 clearance 验算：tan51°·3.4 = 4.2 > 6.9−3.2 = 3.7 → 桥面恰好
越过前墙顶边（与视频一致，桥线贴着前墙顶）。

**结构（全部对齐视频）**：井坑 3.7 + 石环（菱形纹）+ 之字悬浮石径
（强之字摆动）+ 四面等高墙（**前角敞开**：FL/FR 墙截断于 2.6，V 形
开口是桥面/坑可见的前提）+ 墙顶环廊（菱形纹压顶+檐口+垛口）+ FL 端
平台 + 中央柱（双尖拱托架）+ 橙色转子（点击 → 双层行走桥绕竖直轴
90°，桥上角色随转载运，非 0° 时门洞/桥边界封闭）+ FR 橙框门洞（桥
对接）+ 拱门通高隧道（内藏 10 级基座梯）+ 外墙角梯 10 级登顶 +
红衣小孩（尖帽）沿链落后 1.9 跟随 + 镜头随爬升上移 + 章节标题/提示
动态化。墙面朝向色（视频像素采样）：左墙外立面深 0x507ae2、右墙外
立面浅 0x7ea2ff；BoxGeometry 面序 [+x,-x,+y,-y,+z,-z] 勿再放反。

**对比证据**（393×852 用户视口实拍，左=实现 右=视频帧）：
shots/ch2_vs_video_start.png（t=8）、ch2_vs_video_beam.png（桥上态）。

**已确认事实**（逐帧分析 t=0-111，1fps 全帧 + 关键段全分辨率）：
- 出自**纪念碑谷2**：主角 Ro（橙裙深发）+ **红衣小孩**沿链跟随。
  白昼素净背景（≈rgb(251,249,247)），无星/月。
- 已核路线（t=0-45，已实现）：之字径→井沿外沿→基座梯→拱门隧道
  （穿墙）→行走桥（0°）→FR 门洞→墙内换位→FR 外墙角梯（贴前角
  上行）→墙顶环廊（FR→BR→BL→FL）→FL 端平台。t=20-23 转子两连转
  为演示动作。
- **待续（已测绘未实现）**：t=46-111 后角塔段升起（t≈59，随总成
  旋转 90°×2）→ Ro 井沿右行→梯上桥→转载运→BR 角梯下 → 塔段内梯
  登顶 → 全结构沉坑 + 金色花绽放（章节演出）。

**实现文件**：js/chapters/chapter2.js（META/cameraTarget/cameraHalfH/
cameraDir/skyMode='day'/buildWorld/buildMechanic/buildChild/createPlay
——自定义玩法模块，main.js 检测 chapter.createPlay 优先于通用
createGame；#ch=2 切换，默认 #ch=1）。engine 支持 cameraHalfH/cameraDir
章节覆盖；sky 支持 mode:'day'；ui 支持章节标题/提示动态化。

**ch1 回归**：play2/e2e_check/autocheck 全绿（默认 #ch=1）。
